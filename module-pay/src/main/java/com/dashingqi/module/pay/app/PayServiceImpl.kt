...
class PayServiceImpl : PayService {
    override fun performWxPay(strategy: IPayStrategy<WXPayInfoBean>, activity: Activity, payInfoBean: WXPayInfoBean, payCallback: IPayCallback<WXPayInfoBean>) {
        PayUtils.pay(strategy, activity, payInfoBean, payCallback)
    }

    override fun performAliPay(strategy: IPayStrategy<AliPayBean>, activity: Activity, payInfoBean: AliPayBean, payCallback: IPayCallback<AliPayBean>) {
        PayUtils.pay(strategy, activity, payInfoBean, payCallback)
    }
